...
int main(int argc, char **argv) {
  ...
  if (generate_parse_graph) {
    ...
    if (ast_dot_path != nullptr) {
      ...
      dot_stream.open(ast_dot_path);
      if (!dot_stream.is_open()) {
        std::cerr << "\033[1mmjavac: \033[31merror:\033[0m " << ast_dot_path << ": " << strerror(errno) << std::endl;
    ...
    if (ast_graph_path != nullptr) {
      std::filesystem::path parsed_ast_graph_path(ast_graph_path);
      std::string extension = parsed_ast_graph_path.extension();
      ...
      graph_stream.open(ast_graph_path);
      if (!graph_stream.is_open()) {
        std::cerr << "\033[1mmjavac: \033[31merror:\033[0m " << ast_graph_path << ": " << strerror(errno) << std::endl;
  ...
  if (generate_cfg_path) {
    ...
    if (cfg_dot_path != nullptr) {
      std::ofstream dot_stream;
      dot_stream.open(cfg_dot_path);
      if (!dot_stream.is_open()) {
        std::cerr << "\033[1mmjavac: \033[31merror:\033[0m " << cfg_dot_path << ": " << strerror(errno) << std::endl;
        std::cerr << "\033[1mmjavac: \033[31mfatal error:\033[0m unable to create output dot file" << std::endl;
        exit(EXIT_FAILURE);
      }
      dot_stream << graph_string;
    ...
    if (cfg_graph_path != nullptr) {
      std::filesystem::path parsed_cfg_graph_path(cfg_graph_path);
      std::string extension = parsed_cfg_graph_path.extension();
      ...
      std::ofstream graph_stream;
      graph_stream.open(cfg_graph_path);
      if (!graph_stream.is_open()) {
        std::cerr << "\033[1mmjavac: \033[31merror:\033[0m " << cfg_graph_path << ": " << strerror(errno) << std::endl;
        std::cerr << "\033[1mmjavac: \033[31mfatal error:\033[0m unable to create output graph file" << std::endl;
        exit(EXIT_FAILURE);
      }
