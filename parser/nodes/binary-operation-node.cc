...
BinaryOperationNode::BinaryOperationNode(Node *left, Node *right, std::string binary_operator) {
  this->left = left;
  this->right = right;
  this->binary_operator = binary_operator;
}

void BinaryOperationNode::generate_parse_graph(std::ofstream &stream) const {
